...
## What’s New in SPAN 7.3

- Now you can save the SPAN session or the SPAN parameters in .json files. The SPAN session is a snapshot of the current session, including local file paths and the spectra list loaded. The SPAN parameters is a snapshot of all the parameters, without local file paths. The parameter file can be also shared among different users and loaded to any version of SPAN. This will grant an easy way to reproduce the science results
...
- The emission_lines function of pPXF, which defines the gas emission lines, is now in a separated module in the "Span functions" folder, easy to personalize following exactly the pPXF standard.
- Updated the "Line(s) fitting" task with more robust functions and advanced GUI to perform accurate multi-lines fitting of emission and absorption lines. Now this task will provide as output also the measured flux from the fitted lines and bootstrap simulations for uncertainties estimation
...
- Minor bug fixes of the "Stellar populations and SFH" task
...
![Main panel of SPAN](img/span_macos.png)
