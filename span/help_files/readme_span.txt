SPAN: SPectral ANalysis software V6.3
Daniele Gasparri, Jnay 2025
...
The 1D spectra files required can be generated alo with SPAN, both from long-slit 2D fits images and 3D data cube (e.g. MUSE data) fully reduced and wavelength calibrated.
...
The needed package versions for desktop devices depend on the operating system and whether you use the source code or you want also to CREATE newer versions of the standalone applications. Here are some tips:
	- For Windows and Linux (at least Ubuntu 22.04), the latest working Matplotlib version is 3.8.4 if using SPAN via source code. For macOS is 3.5.3 and you also need Pandas 2.0.0.
    	- If you want to generate new executable versions, using auto_py_to_exe (wrapper to pyinstaller) for Windows and Linux, the latest working version of Matplotlib are 3.8.4 for Windows and 3.4.3 for Linux. 
	- For MacOS users, if you want to generate new executable version, (using py2app; I've never succeed with auto_py_to_exe or directly with pyinstaller) you have to use the Pandas 2.0.0 version and the Matplotlib 3.5.3 version.

    To automatically install the Python packages required and the proper versions, you can also run the script file "install_modules_span.sh" in the terminal, if you are using Linux or MacOS. First you need to make the script executable:
    chmod +x install_modules_span.sh , then:
    ./install_modules_span.sh


    SPAN is optimized and can run also on most Android devices using the Pydroid3 app. The list and versions of packages needed for Android devices using the Pydroid3 app is stored in the 'python_packages_pydroid3.txt' file.
...
****How SPAN works****
SPAN can work with just one 1D spectrum, either in Fits or ASCII format, with the first column to be wavelength and the second flux. The wavelength and the flux of the fits files must be in the extension N.1. 
...
1) xshooter_vis_sample_list_spectra.dat , already preloaded in the main application (you just need to click "Load!"), contains 5 spectra of the central regions of nearby galaxies observed with the VIS arm of ESO XShooter spectrograph at resolution of R = 5000. Wavelength units are in nm. Sampling is linear and the wavelength units to set ("W. scale:") are 'nm';
2) NGC5320_bins.dat, ngc5806_bins.dat and ic3392_bins.dat contain the spatial bins of three spiral galaxies observed with the TNG telescope at resolution FWHM of 3.5 A from 470 to 670 nm. Sampling is logarithmic and wavelengths are in log(A). SPAN will take care of everything; you just need to set "A" in the "W. scale:" option of the "Open Spectra" frame of the main application before clicking "Load!";
3) irtf_K-M_giants_list_spectra.dat contains a sample of 31 giant K and M stellar spectra of the IRTF library from 800 to 5000 nm. Wavelength units are in mu, so you need to check the option "mu" in the "Open Spectra" frame before clicking "Load!".


****Quick start****
If you want to compile the source code, type in the terminal: python3 span_6.3.py. Press the "Load!" button to load the example files.
...
You can analyse the selected spectrum by activating any of the spectral analysys tasks and/or you can modify the spectrum by opening the 'Spectra manipulation' panel in the 'Utility' frame on the right.
...
- 1. Extract spectra from 2D or 3D fits:
    This step is mandatory if you do not still have the 1D spectra needed by SPAN. It allows you to extract 1D spectra either from 2D fully reduced fits images or 3D fully reduced fits images, i.e. data cubed.
    - If you have 2D fits of longslit spectra with the dispersion axis along the X axis of the image, press the 'Long-slit extraction' button. There you can fit the trace, correct for distortion and/or slope and extract a 1D spectrum or a series of 1D spectra binned in order to reach a Signal to Noise (S/N) threshold;
...
Both these extraction routines will save the 1D spectra in the SPAN_results directory and a spectra file list in the main directory of SPAN ready to be loaded.

- 2. Generate a spectra list with 1D spectra. If you already have 1D spectra stored in a folder, or in case you performed the extraction of the spectra with SPAN, you should click now on the button 'Generate a spectra list containing 1D spectra'. You should then browse the folder where you stored your spectra. SPAN will read all the spectra contained in the selected folder and in any eventual subfolder and will create an ASCII files with their names and paths. The spectra list generated will be automatically loaded in the 'Browse the spectra list or one spectrum'. In case you want to load just a single 1D spectrum, you can skip this step.
...
Since version 6.3, all the tasks devoted to the manipulation of the spectra have been grouped in the Spectra manipulation panel, Here is a brief description of this panel.

Here you will find some useful tasks that can be performed on spectra, grouped into the 'Spectra pre-processing,' 'Spectra processing,' and 'Spectra math' frames. Any task executed within these frames modifies the selected spectrum and will have effects of the 'Spectral analysis' frame.
...
The Spectral analysis frame contains the following task: 1) Blackbody fitting, 2) Cross-correlation, 3) Velocity dispersion, 4) Line-strength analysis, 5) Line(s) fitting, 6) Stars and gas kinematics, 7) Stellar populations and SFH. Each task is independent from the others and does not modify the spectra.
...
###Apply the tasks###
Once you are satisfied with your work, you can process the spectra or the single spectrum. The 'Process selected' button will perform all the tasks activated in the Spectra manipulation panel and in the "Spectral analysis" frame, saving the new processed spectrum to a fits file. By default, the program will save intermediate files, one for each activated task in the Spectra manipulation panel. For example, if you have selected rebinning, sigma broadening, and add noise, the program will save a spectrum with rebinning done, a second spectrum with rebinning + sigma broadening applied, and a third with rebinning + sigma broadening + add noise applied.
If you are not interested in saving all the intermediate spectra files, you can deselect the 'Save intermediate files' option at the very bottom of the main panel, and only the spectrum at the end of the selected tasks (if any) will be saved.
...
2) Doppler correction file for the Dopcor task and the 'I have a file' option selected:
...
2) Spectra manipulation panel
    ...
    b) Spectra processing frame
        ...
        c) Normalise to = normalises the spectra to the wavelength provided by the user (in nm);
        ...
        f) Continuum model = models the continuum shape with two options: 1) Simple filtering of the continuum by reducing the spectrum to a very small resolution (R = 50), and 2) polynomial fitting, with the possibility to masks emission/contaminated regions. Both the continuum models can be divided or subtracted to the original spectrum;
    ...
    c) Spectra math
        ...
        g) Add constant = add a constant to the spectra;
        i) Multiply by a constant = multiplies the spectra by a user defined constant value.
        j) Calculate first and second derivatives = automatic calculation of the derivatives of the spectra. This task does not modify the original spectra and the derivative spectra cannot be directly used for spectral analysis.
...
2) DataCube extraction: inspired by the GIST pipeline, this sub-program allows you to extract 1D spectra from MUSE and CALIFA DataCubes using the Voronoi binning (Cappellari et al., 2003). It also allows to visualise the DataCube loaded and donamically create a mask (if needed).
...
The menu bar was introduced in version 4.5 of SPAN, offering several helpful options to enhance your experience with spectral analysis. Here is a detailed overview of some options that you won't find in the main panel:
...
4) Edit --> Clear All Tasks: Immediately deactivates all tasks activated in the main panel during the session, enabling a clean restart;
