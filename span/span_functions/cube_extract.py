...
def spatialMasking_Module(config, cube, preview):
    """
    ...
    if preview ==False and (os.path.isfile(output_file) and not config['GENERAL']['OW_OUTPUT']):
...
def spatialBinning_Module(config, cube, preview, voronoi):
    """
    This function calls the spatialBinning routine specified by the user.
    """
    print("spatialBinning module")
...
def prepareSpectra_Module(config, cube, preview):
    """
    This function calls the prepareSpectra routine specified by the user.
    """
    print("prepareSpectra module")
    ...
    if preview ==False and (not config['GENERAL']['OW_OUTPUT'] and
...
def extract(config, preview, voronoi):
    ...
    """
    # 1) reading the cube
    cube = readData_Module(config)
    if cube == "SKIP":
        return

    # 2) apply mask
    _ = spatialMasking_Module(config, cube, preview)

    # 3) Voronoi binning
    _ = spatialBinning_Module(config, cube, preview, voronoi)

    # 4) Arrange the spectra
    _ = prepareSpectra_Module(config, cube, preview)
